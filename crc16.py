...
def calc(buf: bytes | list[int], buf_len, endian="big", crc16: list[int] = crc16tab) -> int:
    crc = 0x1D0F  # initial value
    if endian == "big":
        for i in range(buf_len):
            index = ((crc >> 8) & 0xFFFF) ^ ((buf[i] >> 8) & 0x00FF)
            crc = ((crc << 8) ^ (crc16[index])) & 0xFFFF
...
def calc_modbus_crc16_bytes(data: bytes | list[int]) -> tuple[int, int]:
...
if __name__ == '__main__':  # Если мы запускаем файл напрямую, а не импортируем
    print(hasattr(A, 'b'))
    # print(calc_modbus_crc16_bytes(b'\x0f\x10\x00\x00\x00 @\xf1\x0f\xff\x0f\x00\x00\x9aE\x00\x00\x82\x00\x00\x00E
    # \x00\'\x00)\x00+\x00*\x00&\x00(\x00:\x00\x16\x00\x01\x00\x00\x00\xff\x0f\x10\x02\x02\x00\x00\x00\x00\x00\x00\x00
    # \x00\x00\x00\x00\x00\x00/\x00\x00\x00\x00\x00\xfe\xfe\xff"\xdb\x07'))
    # print(hex(calc(b'\x01\x00\x02\x00\x02\x02\x08\x00', 8)))
    # print(hex(calc([0x00, 0x01, 0x90, 0x00, 0x02, 0x02, 0x08, 0x00], 8, endian="big")))
    # print(crc16(b'\x0f\x10\x00\x00\x00 @\xf1\x0f\xff\x0f\x00\x00\x9aE\x00\x00\x82\x00\x00\x00E\x00\'\x00)\x00+\x00*
    # \x00&\x00(\x00:\x00\x16\x00\x01\x00\x00\x00\xff\x0f\x10\x02\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00
    # \x00/\x00\x00\x00\x00\x00\xfe\xfe\xff"\xdb\x07'))
